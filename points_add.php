<div class="col-md-9 p-3">
    <div class="shadow-sm rounded p-4" style="background-color: #3f51b5; color: #f1f1f1;">
        <h4 class="mb-4" style="color: #fdd835;">➕ Add Dream11 Point Rule</h4>

        <?= validation_errors('<div class="alert alert-danger">', '</div>'); ?>

        <form method="post" action="<?= site_url('DozenDreams/pointsAdd') ?>">
            <div class="mb-3">
                <label for="action" class="form-label text-warning">Action</label>
                <input type="text" name="action" class="form-control bg-light text-dark" value="<?= set_value('action') ?>" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="role_id" class="form-select bg-light text-dark" required>
                    ...
                    <?php foreach ($roles as $id => $name): ?>
                        <option value="<?= $id ?>" <?= set_select('role_id', $id) ?>><?= htmlspecialchars($name) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="type_id" class="form-select bg-light text-dark" style="min-width: 200px;" required>
                    ...
                    <?php foreach ($match_types as $id => $name): ?>
                        <option value="<?= $id ?>" <?= set_select('type_id', $id) ?>><?= htmlspecialchars($name) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="points" class="form-control bg-light text-dark" value="<?= set_value('points') ?>" step="any" required>
            </div>

            <button type="submit" class="btn btn-warning">➕ Save Rule</button>
